...
\section{Approaches to Text Classification }
\label{sec:Approaches to Text Classification }
...
\subsection{Vector Space Model Approach}
...
Naive Bayes is a simple class-conditional generative model which builds on the assumption that all data points are conditionally independent given the classes $\mathcal{C}_n$. For each class $c_j$ we can predict it's probability given an input vector $\mathbf{x}$ as:
...
\begin{wrapfigure}{r}{0.5\textwidth}
  ...
  \caption{A simple Decision Tree to determine whether a building is located in New York (NY) or San Francisco (SF). Adapted from R2D3's outstanding visual introduction to Machine Learning at \url{http://www.r2d3.us/visual-intro-to-machine-learning-part-1/}}
\label{fig:decision-tree}
\end{wrapfigure}

A simple but widely used classification as well as regression method are Decision Trees which work by recursively partitioning the input space into dichotomous subregions, producing a binary decision tree. At inference time the resulting tree can then be used to produce an output by navigating through a path from its root to a leaf, answering a binary questions at each node to determine which child node to query next. Figure~\ref{fig:decision-tree} shows the visualization of an exemplified such a tree. Every node represents a threshold in the input space that  the input data is compared against, corresponding to a decision such as ``Is the elevation of this building higher than or equal to 91.9 feet?''. If the answer is ``Yes'' the left child node is queried next, otherwise the right. This procedure is repeated until a leaf with a final answer is reached.
...
Where $\lambda$ is a regularization parameter. Pruning happens after training a full tree as evidence shows that sometimes several splits occur that do not significantly reduce the error, followed by one that does~\cite[Chapter 14.4, p.~664]{Bishop:2006aa}.

\paragraph{Random Forests}
\label{par:Random Forests}

The Random Forests learning algorithm is an \emph{ensemble method} introduced by~\cite{Breiman:aa}. It is based on the concept of Decision Trees as explained above, but instead of a single decision tree a set of trees (a forest) is grown on sets of bootstrapped samples from the input space. Specifically the algorithm grows $b = 1 \ldots B$ random trees by repeating the following steps for $b$ iterations:
...
Where $\mathcal{I}(\cdot)$ is the indicator function. Other voting schemes are of course possible as well.
...
\label{par:Support Vector Machines}

\paragraph{Neural Networks}
\label{par:Neural Networks}
