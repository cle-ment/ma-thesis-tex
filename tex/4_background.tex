...
\begin{equation}
\mathbb{H}(p,q) = -\sum_x p_x \log q_x =-y\log \hat{y}-(1-y)\log (1-\hat{y})
\end{equation}

(see \cite[Chapter 4.3.2, p.~205 ]{Bishop:2006aa} and \cite[Chapter 10.7, p.~251 ]{Alpaydin:2014aa}).
...
Effectively this minimizes the
This yields a well motivated loss

\todo{note on relation to KL-Divergence?}
...
Note that \textquote{Macroaveraging gives equal weight to each class, whereas microaveraging gives equal weight to each per-document classification decision. Because the F1 measure ignores true negatives and its magnitude is mostly determined by the number of true positives, large classes dominate small classes in microaveraging.}~\cite[Chapter 13.6, p.~280]{Manning:2008aa}. Formally this can be defined as follows, with R denoting for Recall and P the Precision.

\begin{equation}
  \text{R}_{\text{micro}} = \frac{\sum_i^K \text{TP}_i }{ \sum_i^K \text{TP}_i + \text{FN}_i } \qquad
  \text{R}_{\text{macro}} = \frac{\sum_i^K \text{R}_i }{ M }
\end{equation}
\begin{equation}
  \text{P}_{\text{micro}} = \frac{\sum_i^K \text{TP}_i }{ \sum_i^K \text{TP}_i + \text{FP}_i } \qquad
  \text{P}_{\text{macro}} = \frac{\sum_i^K \text{P}_i }{ M }
\end{equation}
...
Another common way of evaluating multi-class classifiers is the categorical cross-entropy which, also known as \emph{multi-class log loss}.
...
where $y_{nk} = y_k (\phi_n)$, and $\mathbf{T}$ is an  $N \times K$ matrix of target variables with elements
$t_{nk}$.
...
\subparagraph{Stop words} For creating N-gram models, so-called stop word lists are often used which are lists of frequent words that will be excluded as they do not carry much meaning~\cite[Chapter 1.3.1, p.~7]{Leskovec:2014aa}. The stop-word list used in these experiments is the standard list used for the Scikit-learn framework \cite{Pedregosa:2011aa} which is a list gathered by the University of Glasgow Information Retrieval group \footnote{\url{http://www.gla.ac.uk/schools/computing/research/researchoverview/informationretrieval/}. The full stop word list can be found at \url{http://ir.dcs.gla.ac.uk/resources/linguistic_utils/stop_words} and in the appendix in section \ref{sec:appendix-stopwords}.}.
...
These models have been shown to outperform state of the art N-gram models on various tasks (see e.g. \cite{bengio2003neural} or \cite{Mikolov:2012aa}). An surprising outcome of this research was the fact that these \emph{word vectors} capture many interesting and often subtle semantic regularities and that these can be exploited explicitly in an algebraic manner. When trained on an extensive dataset, one can perform calculations as $v(Paris) - v(France) + v(Germany)$ and the closest vector to the result turns out to be $v(Berlin)$ where $v(\cdot)$ denotes the \emph{word vector} of a word. Figure \ref{fig:word2vec-cities} shows a PCA projection of Skip-gram trained vectors of countries and their capital cities.

A notable \todo{mention GloVe}

Various extensions and variants to such models have been proposed, including architectures based on Recurrent Neural Networks (\cite{Mikolov:2012aa}). Some of the most important variations will are discussed in the following section as they were evaluated in the experiments:

\subparagraph{Negative Sampling}

\subparagraph{Hierarchical Sampling}

\subparagraph{Frequent word sub-sampling}

\paragraph{Distributed representations for documents}

\todo{averaging }
\todo{parse trees (socher}
\todo{doc2vec}
